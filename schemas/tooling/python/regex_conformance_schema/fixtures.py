...
def _profile(root: Path, name: str) -> IdentityProfile:
    return IdentityProfile.from_record(
        load_strict(root / "schemas" / "identity-profiles" / name)
    )
...
def _oracle(root: Path, value: Any) -> dict[str, Any]:
    ...
    try:
        ...
        raise ConformanceDataError(
            "oracle-unavailable", "Node.js was not found"
        ) from error
    except subprocess.TimeoutExpired as error:
        raise ConformanceDataError(
            "oracle-timeout", "Node.js oracle exceeded 10 seconds"
        ) from error
    except subprocess.CalledProcessError as error:
        raise ConformanceDataError(
            "oracle-failed", error.stderr.strip() or "Node.js oracle failed"
        ) from error
...
def _canonical_result(root: Path, value: Any) -> dict[str, Any]:
    ...
    if python != node:
        fail(
            "oracle-disagreement",
            f"Python and Node canonicalization disagree: {python!r} != {node!r}",
        )
    return python


def _content_result(
    root: Path, registry: NamespaceRegistry, case: dict[str, Any]
) -> dict[str, Any]:
    ...
    if python != node:
        fail(
            "oracle-disagreement",
            f"Python and Node content envelopes disagree: {python!r} != {node!r}",
        )
    return {
...
def verify_manifest(root: Path, manifest_path: Path) -> dict[str, int]:
    ...
    for case in manifest["cases"]:
        ...
        if case["kind"] == "jcs":
            ...
            if actual != case["expected"]:
                fail(
                    "fixture-mismatch",
                    f"fixture {case_id!r} does not match its expected canonical result",
                )
            ...
            if actual != case["expected"]:
                fail(
                    "fixture-mismatch",
                    f"fixture {case_id!r} does not match its expected content result",
                )
            results[case_id] = actual
        else:
            try:
                ...
                if error.code != case["error_code"]:
                    fail(
                        "wrong-fixture-error",
                        f"fixture {case_id!r} produced {error.code!r}, expected {case['error_code']!r}",
                    )
            else:
                fail(
                    "missing-fixture-error",
                    f"fixture {case_id!r} unexpectedly projected successfully",
                )
    ...
    for assertion in manifest["assertions"]:
        ...
        field = (
            "content_id" if assertion["kind"].endswith("id") else "canonical_utf8_hex"
        )
        values = [case[field] for case in cases]
        equal_expected = assertion["kind"].startswith("equal")
        passed = (
            len(set(values)) == 1 if equal_expected else len(set(values)) == len(values)
        )
        if not passed:
            fail(
                "fixture-assertion-failed",
                f"{assertion['kind']} failed for {assertion['cases']!r}",
            )
    return counts
